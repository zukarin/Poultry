...
// Fetch all orders with user information and proof image file name
$select_orders_query = "SELECT orders.*, users.username, users.email, products.product_title, proof_images.file_name AS proof_image
                       FROM orders
                       INNER JOIN users ON orders.user_email = users.email
                       INNER JOIN products ON orders.product_id = products.product_id
                       LEFT JOIN proof_images ON orders.user_email = proof_images.user_email
                       AND orders.order_date = proof_images.upload_date";
  ...
  <span style="color: #fafaf9;">Logout</span>
    ...
    <style>
        ...
        button {
            ...
            font-size: 16px;
            color: white;
            background-color:rgb(91, 141, 48);
            border: none;
            border-radius: 5px;
            cursor: pointer;
        ...
        button:hover {
            background-color:rgb(114, 101, 47);
            transform: scale(1.1);
...
while ($order = mysqli_fetch_assoc($result_orders)) {
    ...
    // Display the proof image if available
    if (!empty($order['proof_image'])) {
        echo "<img src='../.website_with_user_acc/proof_images/{$order['proof_image']}' alt='Proof Image' style='max-width: 100px; max-height: 100px;'>";
    } else {
        echo "No Proof Image";
    }
    echo "</td>";
